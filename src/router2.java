...
public class router2 implements ActionListener
{
	String status="";
	public JLabel la1= new JLabel("ROUTER B");
					 ...
					 while (true)
						...
						con=getDBConnection();
	   					...
	   					//st.execute("drop table updatemain");
	   				 	st.execute("select * into updatemain from main where destination<>2002");
	   					String strLine;
						...
						status+=time+" : The 'Router B' has some failure . . .\n\n";
		  				t2.setText(status);
		  				status+=time+" : Data is transfered using subpath(router) . . .\n\n";
		  				t2.setText(status);
			   			JOptionPane.showMessageDialog(jf, "Router B has some failure !\nFile sent through subpath !","Failure",  JOptionPane.INFORMATION_MESSAGE);
						Thread.sleep(2000);
						byte[] byteArray;
		  				Socket router = null;
		  				try
			  			{
							rs=st.executeQuery("select * from main where source=1002");
							int t[]=new int[10];
							int i=0,j,min=0;
							String rip=null;
							while(rs.next())
				   	 ...
				   	 while (true)
				   	 {
						 ...
						 Character c=new Character(dest);
		  				 ...
		  				 try
			  			 {
				  			con=getDBConnection();
				  			st=con.createStatement();
				  			rs=st.executeQuery("select * from router2");
							while(rs.next())
							{
								if(rs.getInt(1)==des)
								{
							...
							if(flag==0)
							{
								rs=st.executeQuery("select * from updatemain where source = 1002");
								rs.next();
								int pno=rs.getInt(3);
								SocketAddress sockaddr=null;
								if(pno==2001)
									sockaddr = new InetSocketAddress(routerip[0],pno);
								else
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()== b)
		{
			try
			{
				String filename = JOptionPane.showInputDialog("Enter the file name","router2_log.txt");
