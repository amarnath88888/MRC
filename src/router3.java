...
public class router3 implements ActionListener
{
	String status="";
	public JLabel la1= new JLabel("ROUTER C");
	   			...
	   			try
		   		{
					 ...
					 while (true)
						...
						con=getDBConnection();
		   				...
		   				st.execute("select * into updatemain from main where destination<>2003");
	   					String strLine;
						...
						status+=time+" : The 'Router C' has some failure . . .\n\n";
		  				t2.setText(status);
		  				status+=time+" : Data is transfered using subpath(router) . . .\n\n";
		  				t2.setText(status);
			   			JOptionPane.showMessageDialog(jf, "Router C has some failure !\nFile sent through subpath !","Failure",  JOptionPane.INFORMATION_MESSAGE);
						Thread.sleep(2000);
						byte[] byteArray;
		  				Socket router = null;
		  				try
			  			{
							rs=st.executeQuery("select * from main where source=1003");
							int t[]=new int[10];
							int i=0,j,min=0;
							String rip=null;
							while(rs.next())
				   	 ...
				   	 while (true)
				   	 {
						 ...
						 byte[] byteArray;
		  				 ...
		  				 try
			  			 {
				  			con=getDBConnection();
				  			st=con.createStatement();
				  			rs=st.executeQuery("select * from router3");
							while(rs.next())
							{
								if(rs.getInt(1)==des)
								{
							...
							if(flag==0)
							{
								rs=st.executeQuery("select * from updatemain where source = 1003");
								rs.next();
								int pno=rs.getInt(3);
